...
# Register cleanup before either build so partial and successful runs remove
# root-level binaries as well as temporary testdata and the baseline worktree.
cleanup() {
	[ -n "$TESTDATA_TMP" ] && rm -rf -- "$TESTDATA_TMP"
	rm -f -- "$CURRENT_BIN" "$REPO_ROOT/.bench-baseline"
